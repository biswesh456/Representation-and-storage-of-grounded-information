...
def main():
    ...
    if args.train_file:
        ...
        files = glob("/home/tcharlot/Representation-and-storage-of-grounded-information/data/merged_meetup/*.csv")
        dialogs, answers = load_prompt(files, tokenizer, "", processing="only_dialog")
        #dict of datasets NOT GOOD -> dataset of train validation
        ds = Dataset.from_list([{"text":d} for d in dialogs])
        
        ds_temp = ds.train_test_split(test_size=0.2)
        test_valid = ds_temp['test'].train_test_split(test_size=0.5)
        data_dialogs = DatasetDict({'train': ds_temp['train'],
                        'test': test_valid['test'],
                        'validation': test_valid['train']})
        
        column_names = data_dialogs["train"].column_names
        text_column_name = "text" if "text" in column_names else column_names[0]
        with accelerator.main_process_first():
            tokenized_datasets = data_dialogs.map(
                tokenize_function,
                batched=True,
    ...
    if overrode_max_train_steps:
        ...
        n_steps = args.max_train_steps * accelerator.num_processes
    
    lr_scheduler = DummyScheduler(name=args.lr_scheduler_type,
        optimizer=optimizer,
        num_warmup_steps=args.num_warmup_steps * accelerator.num_processes,
        num_training_steps=(
            args.max_train_steps
    ...
    print("first set : ", len(train_dataloader), args.gradient_accumulation_steps)
    if overrode_max_train_steps:
        args.max_train_steps = args.num_train_epochs * num_update_steps_per_epoch
    print("second set : ", args.max_train_steps, args.num_train_epochs, num_update_steps_per_epoch)
    # Afterwards we recalculate our number of training epochs
    args.num_train_epochs = 1
    print("third set : ", args.num_train_epochs)
